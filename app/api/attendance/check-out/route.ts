...
export async function POST(req: NextRequest) {
  ...
  // Shift-based early leave
  let scheduledEndTime: Date | null = null;
  let earlyCheckoutMinutes: number | null = null;

  if (employee.shift?.endTime) {
    scheduledEndTime = buildDateWithTime(employee.shift.endTime, now);
    const diff = diffMinutes(scheduledEndTime, now); // positive = left early
    earlyCheckoutMinutes = diff > 0 ? diff : 0;
  }
